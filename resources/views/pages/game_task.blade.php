...
@if(sizeof($tasks) > 0)
    ...
    <div class="col-xs-12">
        <div class="game-task-container" @if($tries > 0) data-task-id="{{$task->id}}" @endif>
            @if($tries == 0)
                <p>{{ __('Quest available<br><small>To participate in the quest, you must purchase attempts</small>')}}</p>
                <a href="javascript:void(0)" class="ll" onclick="load('/tasks')">{{ __('Go to page')}}</a>
            @else
                <p>
                    Задание:<br>
