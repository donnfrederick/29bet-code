    ...
    aiCU: function (e, t) {
        $.on("/stairs", function () {
            ...
            fl = 0,
                ...
                cr = [],
                rws = [
                    20, 19, 19, 17, 19, 15, 17, 12, 11, 19, 10, 9, 8
                ],
            ...
            betAudio = new Audio, winAudio = new Audio, gameAudio = new Audio, loseAudio = new Audio, window.row = function (e, t) {
                        ...
                        if (xhr.status >= 200 && xhr.status < 300) {
                            ...
                            if (response.status === 200) {
                                ...
                                setTimeout(function () {
                                    ...
                                    $('#money_gif__lose').attr('src', 'https://29betbucket.s3.ap-southeast-1.amazonaws.com/assets/img/all/icons/coin-gif.gif');
                        ...
                        if (xhr.status >= 200 && xhr.status < 300) {
                            ...
                            if (response.status === 200) {
                                ...
                                setTimeout(function () {
                                    ...
                                    $('#money_gif__gain').attr('src', 'https://29betbucket.s3.ap-southeast-1.amazonaws.com/assets/img/all/icons/coin-gif.gif');
                ...
                alienBuddy.classList.remove('alien_walking_left', 'alien_walking_right', 'alien_climbing', 'alien_hurt');


                // alienBuddy.style.transition = 'none';
                // Set the initial left and top positions
                alienBuddy.style.left = '-15px';
                alienBuddy.style.top = '427px';
                ...
                winAudio.src = isAudioGame ? "/assets/media/win.mp3" : "", isSoundOn ? winAudio.play() : null, setTimeout(function () {
                    s = !1
                }, 350), 1 == demo || (sendDrop(t), validateTask(t)), $(".outcome-window__coeff").text("x" + fm[t - 1]), $(".outcome-window_won__sum").text(parseFloat(fp).toFixed(2)), $(".outcome-window").fadeIn(200), clI = setInterval(() => {
                ...
                if (!$(this).hasClass("stairs-block-disabled")) {
                    ...
                    if (fg[t][e] == 0) {
                        ...
                        a ? $("#cf_profit").html(r) : ($('#play').attr('disabled', false), setBetText(gBtT("pegar") + '<br><span id="cf_profit">' + r + '</span>&nbsp;<i class="fad fa-coins"></i>'), a = !0), setTimeout(function () {
                            ...
                            13 === t ? (cl = 0, take()) : (t += 1, cg += 1, cf += cfI, row(t - 1, !1), row(t, !0), cl = 0), gameAudio.src = isAudioGame ? "/assets/media/game.mp3" : "", isSoundOn ? gameAudio.play() : null
                    } else uapts(0), cl = 1, t += 1, gBmR(nb - 1), loseAudio.src = isAudioGame ? "/assets/media/redzone.mp3" : "",
                        isSoundOn ? loseAudio.play() : null, isDemo || sendDrop(i), $('.outcome-window-lose').delay(6000).fadeIn(1000), cli = setInterval(() => {
...
stairsBlocks.forEach(function (block) {
    block.addEventListener('click', function (event) {
        ...
        var offsetX = (event.clientX - containerRect.left) - 35;
        var offsetY = (event.clientY - containerRect.top);

        alienBuddy.classList.remove('alien_idle', 'alien_walking_left', 'alien_walking_right', 'alien_climbing', 'alien_hurt');
        ...
        var walkDuration = isFastAnimation ? 1500 : 5000;
        // if ($('.outcome-window').hasClass(('stairs-bad'))) {
        //     console.log("Outcome window has outcome-window-lose class");
        //     alienBuddy.classList.add('alien_hurt');
        //     alienBuddy.style.transition = 'none';
        // }  
        // console.log("Continue");
        // Lakad animation ni buddy
        ...
        setTimeout(function() {
            if (isClimbingEnabled) {
                ...
                var climbDuration = 2000;
                alienBuddy.style.transition = `top ${climbDuration}ms linear`;
                alienBuddy.style.top = offsetY - 2.5 * blockRect.height + 'px';
