            ...
            betAudio = new Audio, minesAudio = new Audio, winAudio = new Audio, loseAudio = new Audio, window.setMode = function(e, a) {
                ...
                if (b < 1) {
                    ...
                    if (e < 3 || e > 24) {
                        ...
                        fg = g;
                        clear(), $(".outcome-window").fadeOut(200), clearInterval(clI), $(".outcome-window-lose").fadeOut(200), clearInterval(cli), betAudio.src = isAudioGame ? "https://cdn.29bet.com/assets/media/bet.mp3" : "", isSoundOn ? betAudio.play(): null, $("#play").attr("onclick", "take()").attr('disabled', true), setTimeout(function() {
                            l = !1
                ...
                winAudio.src = isAudioGame ? "https://cdn.29bet.com/assets/media/win.mp3" : "", isSoundOn ? winAudio.play(): null, 1 == demo || (validateTask(d), $(".outcome-window__coeff").text("x" + ml[fl - 1]), $(".outcome-window_won__sum").text(parseFloat(fp).toFixed(2)), $(".outcome-window").fadeIn(200), clI = setInterval(() => {$(".outcome-window").fadeOut(200);clearInterval(clI)}, 3000)), isDemo && isGuest() && showDemoTooltip(), 1 == demo && ($(".outcome-window__coeff-demo").text("x" + ml[fl - 1]), $(".outcome-window_won__sum").text(parseFloat(fp).toFixed(2))), swap(!0), clear(), updateBalanceN(), displayGrid(fg), l = !1, _disableDemo = !1, fg = [], fm = [], fl = 0
            ...
            $("#change_bombs").on("click", function() {
                ...
                if (!cd === !0) {
                    if (!($(this).hasClass("mine_disabled") || t || $(this).hasClass("mine_safe"))) {
                        ...
                        } else setBetText(gBtT("jogar")), $('#play').attr("disabled", true), cl = 0, cd = 1, validateTask(d), loseAudio.src = isAudioGame ? "https://cdn.29bet.com/assets/media/bomb.mp3" : "", isSoundOn ? loseAudio.play() : null, l = !1, uapts(0), fg = [], fm = [], fl = 0, fp = 0, $("*[data-grid-id=" + a + "]").toggleClass("mine_bomb", !0), $("*[data-diamond=" + n + "]").toggleClass("cf_active", !1), setTimeout(function() {
                            updateBalanceN(), swap(!0), clear(), displayGrid(nfg), $(".outcome-window-lose").fadeIn(200), cli = setInterval(() => {$(".outcome-window-lose").fadeOut(200);clearInterval(cli)}, 3000)
                        }, 1e3), setTimeout(() => {$('#play').attr('onclick', 'mines()').attr("disabled", false)}, 1000), $('.slick-slider').slick('slickGoTo', 0)
                    }
                }
