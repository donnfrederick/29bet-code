const Roulette = (function () {
    ...
    class Roulette {
        ...
        rotateTo(block, options) {
            ...
            let prize = Number.isNaN(numBlock) ? this.findPrize({
                element: block
            }) : this.findPrize({
                index: numBlock
            });
            ...
            let {
                tracks = 0, time = 0, random = true, backward = false
            } = options || {};
            time |= 0;
            tracks |= 0;
            if (time) {
        ...
        //     if (this.shdStp !== true) {
        //         this.shdStp = true;
        //         this.container.dispatchEvent(new CustomEvent(rotationStopEventName, {
        //             detail: {
        //                 prize: this.selectedPrize
        //             }
        //         }));
        //         let currentMarginLeft = parseFloat(this.firstBlock.wrapper.style.marginLeft || "0px");
        //         let overshoot = currentMarginLeft + this.center;


        //         if (Math.abs(overshoot) > 0) {
        //             const transitionDuration = 500;
        //             const transitionTimingFunction = 'ease-in-out';
        //             this.firstBlock.wrapper.style.transition = `margin-left ${transitionDuration}ms ${transitionTimingFunction}`;
        //             this.firstBlock.wrapper.style.marginLeft = "5px";
        //             setTimeout(() => {
        //                 this.prizes.forEach(prize => this.list.appendChild(prize.wrapper));
        //             }, transitionDuration);
        //         }
        //         if (this.rotates) {
        ...
        stop() {
            ...
            // pang last block trans
            if (lastBlockBeforeRotation) {
                lastBlockBeforeRotation.wrapper.style.transition = "margin-left 2s in-ease";
            }
            if (this.rotates) {
                clearInterval(rotationTokens.get(this));
                rotationTokens.set(this, -1);
        ...
        get firstBlock() {
            return this.findPrize({
                element: this.list.querySelector(`:scope > .${roulettePrizeClass} > *`)
            });
        }
    ...
    function rotateForward(pixels) {
        ...
        lastBlockBeforeRotation = this.selectedPrize;
    ...
    function rotateByTracks(prize, tracks, random, backward) {
        ...
        // Calculate the distance to recenter after the rotation
        let recenterDistance = (length % this.width) - this.center;

        this.rotate(length);

        // Check if recentering is needed and transition the wheel to the center
        if (Math.abs(recenterDistance) > 0) {
            setTimeout(() => {
                this.firstBlock.wrapper.style.transition = "margin-left 0.5s ease"; // Smooth transition
                this.firstBlock.wrapper.style.marginLeft = "0px";
            }, 1000); // Adjust the delay as needed to ensure it happens after the rotation
        }
    }
